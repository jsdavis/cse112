...
!function($) {
  ...
  function isInputEventSupported(eventName) {
    let el = document.createElement('input'), evName = 'on' + eventName, isSupported = evName in el;
    return isSupported || (el.setAttribute(evName, 'return;'), isSupported = 'function' == typeof el[evName]),
        el = null, isSupported;
  }
  function isInputTypeSupported(inputType) {
    let isSupported = 'text' === inputType || 'tel' === inputType || 'password' === inputType;
    if (!isSupported) {
  ...
  function resolveAlias(aliasStr, options, opts) {
    const aliasDefinition = opts.aliases[aliasStr];
  ...
  function importAttributeOptions(npt, opts, userOptions) {
    function importOption(option) {
      let optionData = npt.getAttribute('data-inputmask-' + option.toLowerCase());
    ...
    let attrOptions = npt.getAttribute('data-inputmask');
    if (attrOptions && '' !== attrOptions) try {
      attrOptions = attrOptions.replace(new RegExp('\'', 'g'), '"');
      const dataoptions = $.parseJSON('{' + attrOptions + '}');
  ...
  function generateMaskSet(opts, nocache) {
    function analyseMask(mask) {
      ...
      function insertTestDefinition(mtoken, element, position) {
        const maskdef = opts.definitions[element];
        position = void 0 !== position ? position : mtoken.matches.length;
        let prevMatch = mtoken.matches[position - 1];
        if (maskdef && !escaped) {
          maskdef.placeholder = $.isFunction(maskdef.placeholder) ? maskdef.placeholder.call(this, opts) : maskdef.placeholder;
          for (let prevalidators = maskdef.prevalidator, prevalidatorsL = prevalidators ? prevalidators.length : 0, i = 1; i < maskdef.cardinality; i++) {
      ...
      function defaultCase() {
        if (openenings.length > 0) {
          if (currentOpeningToken = openenings[openenings.length - 1], maskCurrentToken(m, currentOpeningToken, lastMatch, !currentOpeningToken.isAlternator),
            ...
            for (let mndx = 0; mndx < alternator.matches.length; mndx++) alternator.matches[mndx].isGroup = !1;
      ...
      for (var match, m, openingToken, currentOpeningToken, alternator, lastMatch, groupToken, tokenizer = /(?:[?*+]|\{[0-9\+\*]+(?:,[0-9\+\*]*)?\})|[^.?*+^${[]()|\\]+|./g, escaped = !1, currentToken = new MaskToken(), openenings = [], maskTokens = []; match = tokenizer.exec(mask); ) if (m = match[0],
            ...
            case opts.quantifiermarker.start:
              ...
              if (('*' === mq1 || '+' === mq1) && (mq0 = '*' === mq1 ? 0 : 1), quantifier.quantifier = {
                ...
                const matches = openenings[openenings.length - 1].matches;
    ...
    function generateMask(mask, metadata) {
      ...
      if (1 === mask.length && opts.greedy === !1 && 0 !== opts.repeat && (opts.placeholder = ''),
        ...
        const repeatStart = '*' === opts.repeat ? 0 : '+' === opts.repeat ? 1 : opts.repeat;
    ...
    if ($.isFunction(opts.mask) && (opts.mask = opts.mask.call(this, opts)), $.isArray(opts.mask)) {
      if (opts.mask.length > 1) {
        ...
        let altMask = '(';
        return $.each(opts.numericInput ? opts.mask.reverse() : opts.mask, (ndx, msk) => {
  ...
  function maskScope(actionObj, maskset, opts) {
    function getMaskTemplate(baseOnInput, minimalPos, includeInput) {
      ...
      let ndxIntlzr, test, testPos, maskTemplate = [], pos = 0;
      do {
        if (baseOnInput === !0 && getMaskSet().validPositions[pos]) {
          const validPos = getMaskSet().validPositions[pos];
    ...
    function resetMaskSet(soft) {
      const maskset = getMaskSet();
      maskset.buffer = void 0, maskset.tests = {}, soft !== !0 && (maskset._buffer = void 0,
            maskset.validPositions = {}, maskset.p = 0);
    }
    function getLastValidPosition(closestTo, strict) {
      let maskset = getMaskSet(), lastValidPosition = -1, valids = maskset.validPositions;
      void 0 === closestTo && (closestTo = -1);
      let before = lastValidPosition, after = lastValidPosition;
      for (const posNdx in valids) {
        const psNdx = parseInt(posNdx);
    ...
    function setValidPosition(pos, validTest, fromSetValid) {
      if (opts.insertMode && void 0 !== getMaskSet().validPositions[pos] && void 0 === fromSetValid) {
        let i, positionsClone = $.extend(!0, {}, getMaskSet().validPositions), lvp = getLastValidPosition();
        for (i = pos; lvp >= i; i++) delete getMaskSet().validPositions[i];
        getMaskSet().validPositions[pos] = validTest;
        let j, valid = !0, vps = getMaskSet().validPositions;
        for (i = j = pos; lvp >= i; i++) {
          const t = positionsClone[i];
          if (void 0 !== t) for (let posMatch = j, prevPosMatch = -1; posMatch < getMaskLength() && (null == t.match.fn && vps[i] && (vps[i].match.optionalQuantifier === !0 || vps[i].match.optionality === !0) || null != t.match.fn); ) {
    ...
    function stripValidPositions(start, end, nocheck, strict) {
      ...
      for (resetMaskSet(!0), i = startPos + 1; i <= getLastValidPosition(); ) {
        ...
        const s = getMaskSet().validPositions[startPos];
        startPos > i && (i = startPos + 1);
        const t = getMaskSet().validPositions[i];
      ...
      let lvp = getLastValidPosition(), ml = getMaskLength();
    ...
    function getTestTemplate(pos, ndxIntlzr, tstPs) {
      let testPos = getMaskSet().validPositions[pos];
      if (void 0 === testPos) for (let testPositions = getTests(pos, ndxIntlzr, tstPs), lvp = getLastValidPosition(), lvTest = getMaskSet().validPositions[lvp] || getTests(0)[0], lvTestAltArr = void 0 !== lvTest.alternation ? lvTest.locator[lvTest.alternation].toString().split(',') : [], ndx = 0; ndx < testPositions.length && (testPos = testPositions[ndx],
    ...
    function getTests(pos, ndxIntlzr, tstPs, cacheable) {
      function resolveTestFromToken(maskToken, ndxInitializer, loopNdx, quantifierRecurse) {
        function handleMatch(match, loopNdx, quantifierRecurse) {
          ...
          if (void 0 !== match.matches) {
            if (match.isGroup && quantifierRecurse !== match) {
              ...
              let maltMatches, alternateToken = match, malternateMatches = [], currentMatches = matches.slice(), loopNdxCnt = loopNdx.length, altIndex = ndxInitializer.length > 0 ? ndxInitializer.shift() : -1;
              if (-1 === altIndex || 'string' == typeof altIndex) {
                let currentPos = testPos, ndxInitializerClone = ndxInitializer.slice(), altIndexArr = [];
                'string' == typeof altIndex && (altIndexArr = altIndex.split(','));
                for (let amndx = 0; amndx < alternateToken.matches.length; amndx++) {
                    if (matches = [], match = handleMatch(alternateToken.matches[amndx], [amndx].concat(loopNdx), quantifierRecurse) || match,
                                    match !== !0 && void 0 !== match && altIndexArr[altIndexArr.length - 1] < alternateToken.matches.length) {
                        const ntndx = maskToken.matches.indexOf(match) + 1;
                        maskToken.matches.length > ntndx && (match = handleMatch(maskToken.matches[ntndx], [ntndx].concat(loopNdx.slice(1, loopNdx.length)), quantifierRecurse),
                                        match && (altIndexArr.push(ntndx.toString()), $.each(matches, (ndx, lmnt) => {
                                          lmnt.alternation = loopNdx.length - 1;
                                        })));
                      }
                    maltMatches = matches.slice(), testPos = currentPos, matches = [];
                    for (let i = 0; i < ndxInitializerClone.length; i++) ndxInitializer[i] = ndxInitializerClone[i];
                ...
                'string' == typeof altIndex && (malternateMatches = $.map(malternateMatches, (lmnt, ndx) => {
                    if (isFinite(ndx)) {
                        let mamatch, alternation = lmnt.alternation, altLocArr = lmnt.locator[alternation].toString().split(',');
                        lmnt.locator[alternation] = void 0, lmnt.alternation = void 0;
                        for (let alndx = 0; alndx < altLocArr.length; alndx++) mamatch = -1 !== $.inArray(altLocArr[alndx], altIndexArr),
              ...
              if (match) return !0;
            } else if (match.isQuantifier && quantifierRecurse !== maskToken.matches[$.inArray(match, maskToken.matches) - 1]) for (let qt = match, qndx = ndxInitializer.length > 0 ? ndxInitializer.shift() : 0; qndx < (isNaN(qt.quantifier.max) ? qndx + 1 : qt.quantifier.max) && pos >= testPos; qndx++) {
              const tokenGroup = maskToken.matches[$.inArray(qt, maskToken.matches) - 1];
        ...
        for (let tndx = ndxInitializer.length > 0 ? ndxInitializer.shift() : 0; tndx < maskToken.matches.length; tndx++) if (maskToken.matches[tndx].isQuantifier !== !0) {
    ...
    function isValid(pos, c, strict, fromSetValid) {
      function _isValid(position, c, strict, fromSetValid) {
        let rslt = !1;
        return $.each(getTests(position), (ndx, tst) => {
          for (var test = tst.match, loopend = c ? 1 : 0, chrs = '', i = test.cardinality; i > loopend; i--) chrs += getBufferElement(position - (i - 1));
          if (c && (chrs += c), rslt = null != test.fn ? test.fn.test(chrs, getMaskSet(), position, strict, opts) : c !== test.def && c !== opts.skipOptionalPartCharacter || '' === test.def ? !1 : {
            c: test.def,
            pos: position,
          }, rslt !== !1) {
            let elem = void 0 !== rslt.c ? rslt.c : c;
            elem = elem === opts.skipOptionalPartCharacter && null === test.fn ? test.def : elem;
            let validatedPos = position, possibleModifiedBuffer = getBuffer();
            if (void 0 !== rslt.remove && ($.isArray(rslt.remove) || (rslt.remove = [rslt.remove]),
                        $.each(rslt.remove.sort((a, b) => {
                          return b - a;
                        }), (ndx, lmnt) => {
                          stripValidPositions(lmnt, lmnt + 1, !0);
                        })), void 0 !== rslt.insert && ($.isArray(rslt.insert) || (rslt.insert = [rslt.insert]),
                        $.each(rslt.insert.sort((a, b) => {
                          return a - b;
                        }), (ndx, lmnt) => {
                          isValid(lmnt.pos, lmnt.c, !0);
                        })), rslt.refreshFromBuffer) {
              const refresh = rslt.refreshFromBuffer;
      ...
      function alternate(pos, c, strict, fromSetValid) {
        ...
        if (void 0 !== alternation) {
          ...
          for (let decisionPos in getMaskSet().validPositions) if (decisionPos = parseInt(decisionPos),
                    altPos = getMaskSet().validPositions[decisionPos], decisionPos >= lastAlt && void 0 !== altPos.alternation) {
            let altNdxs = getMaskSet().validPositions[lastAlt].locator[alternation].toString().split(','), decisionTaker = altPos.locator[alternation] || altNdxs[0];
            decisionTaker.length > 0 && (decisionTaker = decisionTaker.split(',')[0]);
            for (let mndx = 0; mndx < altNdxs.length; mndx++) if (decisionTaker < altNdxs[mndx]) {
      ...
      function trackbackAlternations(originalPos, newPos) {
        for (var vp = getMaskSet().validPositions[newPos], targetLocator = vp.locator, tll = targetLocator.length, ps = originalPos; newPos > ps; ps++) if (!isMask(ps)) {
          ...
          $.each(tests, (ndx, tst) => {
            for (let i = 0; tll > i; i++) tst.locator[i] && checkAlternationMatch(tst.locator[i].toString().split(','), targetLocator[i].toString().split(',')) && i > equality && (equality = i,
                        bestMatch = tst);
      ...
      let maskPos = pos, result = !1, positionsClone = $.extend(!0, {}, getMaskSet().validPositions);
      if (maskPos < getMaskLength() && (getBuffer(), result = _isValid(maskPos, c, strict, fromSetValid),
            (!strict || fromSetValid) && result === !1)) {
        const currentPosValid = getMaskSet().validPositions[maskPos];
        if (!currentPosValid || null !== currentPosValid.match.fn || currentPosValid.match.def !== c && c !== opts.skipOptionalPartCharacter) {
          if ((opts.insertMode || void 0 === getMaskSet().validPositions[seekNext(maskPos)]) && !isMask(maskPos)) for (let nPos = maskPos + 1, snPos = seekNext(maskPos); snPos >= nPos; nPos++) if (result = _isValid(nPos, c, strict, fromSetValid),
                    result !== !1) {
      ...
      if (result === !1 && opts.keepStatic && isComplete(buffer) && (result = alternate(pos, c, strict, fromSetValid)),
        ...
        resetMaskSet(!0);
        const postValidResult = opts.postValidation(getBuffer(), opts);
        if (postValidResult) {
          if (postValidResult.refreshFromBuffer) {
            const refresh = postValidResult.refreshFromBuffer;
    ...
    function getMaskLength() {
      ...
      let pos, lvp = getLastValidPosition(), testPos = getMaskSet().validPositions[lvp], ndxIntlzr = void 0 !== testPos ? testPos.locator.slice() : void 0;
      for (pos = lvp + 1; void 0 === testPos || null !== testPos.match.fn || null === testPos.match.fn && '' !== testPos.match.def; pos++) testPos = getTestTemplate(pos, ndxIntlzr, pos - 1),
            ndxIntlzr = testPos.locator.slice();
      const lastTest = getTest(pos - 1);
      return maskLength = '' !== lastTest.def ? pos : pos - 1, void 0 === maxLength || maxLength > maskLength ? maskLength : maxLength;
    }
    function seekNext(pos, newBlock) {
      const maskL = getMaskLength();
    ...
    function writeBuffer(input, buffer, caretPos, event, triggerInputEvent) {
      if (event && $.isFunction(opts.onBeforeWrite)) {
        const result = opts.onBeforeWrite.call(input, event, buffer, caretPos, opts);
        if (result) {
    ...
    function checkVal(input, writeOut, strict, nptvl) {
      function isTemplateMatch() {
        let isMatch = !1, charCodeNdx = getBufferTemplate().slice(initialNdx, seekNext(initialNdx)).join('').indexOf(charCodes);
        if (-1 !== charCodeNdx && !isMask(initialNdx)) {
          isMatch = !0;
          for (let bufferTemplateArr = getBufferTemplate().slice(initialNdx, initialNdx + charCodeNdx), i = 0; i < bufferTemplateArr.length; i++) if (' ' !== bufferTemplateArr[i]) {
            isMatch = !1;
            break;
          }
        }
      ...
      if (resetMaskSet(), getMaskSet().p = seekNext(-1), !strict) if (opts.autoUnmask !== !0) {
        let staticInput = getBufferTemplate().slice(0, seekNext(-1)).join(''), matches = inputValue.join('').match(new RegExp('^' + Inputmask.escapeRegex(staticInput), 'g'));
      ...
      $.each(inputValue, (ndx, charCode) => {
        ...
        let lvp = getLastValidPosition(void 0, !0), lvTest = getMaskSet().validPositions[lvp], nextTest = getTestTemplate(lvp + 1, lvTest ? lvTest.locator.slice() : void 0, lvp);
        if (!isTemplateMatch() || strict || opts.autoUnmask) {
          const pos = strict ? ndx : null == nextTest.match.fn && nextTest.match.optionality && lvp + 1 < getMaskSet().p ? lvp + 1 : getMaskSet().p;
    ...
    function unmaskedvalue(input) {
      ...
      let umValue = [], vps = getMaskSet().validPositions;
      for (const pndx in vps) vps[pndx].match && null != vps[pndx].match.fn && umValue.push(vps[pndx].input);
      let unmaskedValue = 0 === umValue.length ? null : (isRTL ? umValue.reverse() : umValue).join('');
      if (null !== unmaskedValue) {
        const bufferValue = (isRTL ? getBuffer().slice().reverse() : getBuffer()).join('');
    ...
    function caret(input, begin, end) {
      function translatePosition(pos) {
        if (isRTL && 'number' == typeof pos && (!opts.greedy || '' !== opts.placeholder)) {
          const bffrLght = getBuffer().join('').length;
          pos = bffrLght - pos;
        }
        return pos;
      }
      ...
      const scrollCalc = (input.ownerDocument.defaultView || window).getComputedStyle(input, null).fontSize.replace('px', '') * end;
    ...
    function determineLastRequiredPosition(returnDefinition) {
      let pos, testPos, buffer = getBuffer(), bl = buffer.length, lvp = getLastValidPosition(), positions = {}, lvTest = getMaskSet().validPositions[lvp], ndxIntlzr = void 0 !== lvTest ? lvTest.locator.slice() : void 0;
      for (pos = lvp + 1; pos < buffer.length; pos++) testPos = getTestTemplate(pos, ndxIntlzr, pos - 1),
            ndxIntlzr = testPos.locator.slice(), positions[pos] = $.extend(!0, {}, testPos);
      const lvTestAlt = lvTest && void 0 !== lvTest.alternation ? lvTest.locator[lvTest.alternation] : void 0;
    ...
    function isComplete(buffer) {
      ...
      let complete = !1, lrp = determineLastRequiredPosition(!0), aml = seekPrevious(lrp.l);
      if (void 0 === lrp.def || lrp.def.newBlockMarker || lrp.def.optionality || lrp.def.optionalQuantifier) {
        complete = !0;
        for (let i = 0; aml >= i; i++) {
          const test = getTestTemplate(i).match;
    ...
    function wrapEvent(eventHandler) {
      return function(e) {
        ...
        if (void 0 === this.inputmask) {
          const imOpts = $.data(this, '_inputmask_opts');
    ...
    function patchValueProperty(npt) {
      function patchValhook(type) {
        if ($.valHooks && void 0 === $.valHooks[type] || $.valHooks[type].inputmaskpatch !== !0) {
          let valhookGet = $.valHooks[type] && $.valHooks[type].get ? $.valHooks[type].get : function(elem) {
              return elem.value;
          ...
          $.valHooks[type] = {
            get: function(elem) {
              if (elem.inputmask) {
                ...
                let result = valhookGet(elem), maskset = elem.inputmask.maskset, bufferTemplate = maskset._buffer;
                return bufferTemplate = bufferTemplate ? bufferTemplate.join('') : '', result !== bufferTemplate ? result : '';
              }
              return valhookGet(elem);
            },
            set: function(elem, value) {
              let result, $elem = $(elem);
      ...
      function installNativeValueSetFallback(npt) {
        $(npt).on('mouseenter.inputmask', wrapEvent(function(event) {
          let $input = $(this), input = this, value = input.inputmask._valueGet();
    ...
    function handleRemove(input, k, pos, strict) {
      function generalize() {
        if (opts.keepStatic) {
          resetMaskSet(!0);
          let lastAlt, validInputs = [], positionsClone = $.extend(!0, {}, getMaskSet().validPositions);
          for (lastAlt = getLastValidPosition(); lastAlt >= 0; lastAlt--) {
            const validPos = getMaskSet().validPositions[lastAlt];
      ...
      const lvp = getLastValidPosition(pos.begin);
      lvp < pos.begin ? (-1 === lvp && resetMaskSet(), getMaskSet().p = seekNext(lvp)) : strict !== !0 && (getMaskSet().p = pos.begin);
    }
    function keydownEvent(e) {
      let input = this, $input = $(input), k = e.keyCode, pos = caret(input);
      k === Inputmask.keyCode.BACKSPACE || k === Inputmask.keyCode.DELETE || iphone && 127 === k || e.ctrlKey && 88 === k && !isInputEventSupported('cut') ? (e.preventDefault(),
            88 === k && (undoValue = getBuffer().join('')), handleRemove(input, k, pos), writeBuffer(input, getBuffer(), getMaskSet().p, e, undoValue !== getBuffer().join('')),
            input.inputmask._valueGet() === getBufferTemplate().join('') ? $input.trigger('cleared') : isComplete(getBuffer()) === !0 && $input.trigger('complete'),
            opts.showTooltip && $input.prop('title', getMaskSet().mask)) : k === Inputmask.keyCode.END || k === Inputmask.keyCode.PAGE_DOWN ? setTimeout(() => {
              let caretPos = seekNext(getLastValidPosition());
            ...
            caret(input, pos.begin, pos.end))) : opts.insertMode !== !1 || e.shiftKey || (k === Inputmask.keyCode.RIGHT ? setTimeout(() => {
              const caretPos = caret(input);
              caret(input, caretPos.begin);
            }, 0) : k === Inputmask.keyCode.LEFT && setTimeout(() => {
              const caretPos = caret(input);
    ...
    function keypressEvent(e, checkval, writeOut, strict, ndx) {
      let input = this, $input = $(input), k = e.which || e.charCode || e.keyCode;
      if (!(checkval === !0 || e.ctrlKey && e.altKey) && (e.ctrlKey || e.metaKey || ignorable)) return k === Inputmask.keyCode.ENTER && undoValue !== getBuffer().join('') && setTimeout(() => {
        $input.trigger('change'), undoValue = getBuffer().join('');
      }, 0), !0;
      if (k) {
        46 === k && e.shiftKey === !1 && ',' === opts.radixPoint && (k = 44);
        let forwardPosition, pos = checkval ? {
            begin: ndx,
            end: ndx,
        ...
        let p = isRTL && !isSlctn ? pos.end : pos.begin, valResult = isValid(p, c, strict);
        if (valResult !== !1) {
          if (valResult !== !0 && (p = void 0 !== valResult.pos ? valResult.pos : p, c = void 0 !== valResult.c ? valResult.c : c),
                    resetMaskSet(!0), void 0 !== valResult.caret) forwardPosition = valResult.caret; else {
            const vps = getMaskSet().validPositions;
        ...
        if (writeOut !== !1) {
          ...
          if (setTimeout(() => {
            ...
            writeBuffer(input, buffer, checkval ? void 0 : opts.numericInput ? seekPrevious(forwardPosition) : forwardPosition, e, checkval !== !0),
                        checkval !== !0 && setTimeout(() => {
                          isComplete(buffer) === !0 && $input.trigger('complete');
                        }, 0);
          } else isSlctn && (getMaskSet().buffer = void 0, getMaskSet().validPositions = getMaskSet().undoPositions);
        } else isSlctn && (getMaskSet().buffer = void 0, getMaskSet().validPositions = getMaskSet().undoPositions);
        if (opts.showTooltip && $input.prop('title', getMaskSet().mask), checkval && $.isFunction(opts.onBeforeWrite)) {
          const result = opts.onBeforeWrite.call(this, e, getBuffer(), forwardPosition, opts);
    ...
    function pasteEvent(e) {
      let input = this, $input = $(input), inputValue = input.inputmask._valueGet(!0), caretPos = caret(input);
      if ('propertychange' === e.type && input.inputmask._valueGet().length <= getMaskLength()) return !0;
      if ('paste' === e.type) {
        let valueBeforeCaret = inputValue.substr(0, caretPos.begin), valueAfterCaret = inputValue.substr(caretPos.end, inputValue.length);
        valueBeforeCaret === getBufferTemplate().slice(0, caretPos.begin).join('') && (valueBeforeCaret = ''),
                valueAfterCaret === getBufferTemplate().slice(caretPos.end).join('') && (valueAfterCaret = ''),
                window.clipboardData && window.clipboardData.getData ? inputValue = valueBeforeCaret + window.clipboardData.getData('Text') + valueAfterCaret : e.originalEvent && e.originalEvent.clipboardData && e.originalEvent.clipboardData.getData && (inputValue = valueBeforeCaret + e.originalEvent.clipboardData.getData('text/plain') + valueAfterCaret);
      }
      let pasteValue = inputValue;
    ...
    function compositionUpdateEvent(e) {
      ...
      setTimeout(() => {
        const forwardPosition = getMaskSet().p;
        writeBuffer(input, getBuffer(), opts.numericInput ? seekPrevious(forwardPosition) : forwardPosition);
      }, 0), compositionData = e.originalEvent.data;
    }
    function compositionEndEvent(e) {}
    function setValueEvent(e) {
      let input = this, value = input.inputmask._valueGet();
    ...
    function focusEvent(e) {
      let input = this, nptValue = input.inputmask._valueGet();
      opts.showMaskOnFocus && (!opts.showMaskOnHover || opts.showMaskOnHover && '' === nptValue) ? input.inputmask._valueGet() !== getBuffer().join('') && writeBuffer(input, getBuffer(), seekNext(getLastValidPosition())) : mouseEnter === !1 && caret(input, seekNext(getLastValidPosition())),
            opts.positionCaretOnTab === !0 && setTimeout(() => {
    ...
    function mouseleaveEvent(e) {
      ...
      if (mouseEnter = !1, opts.clearMaskOnLostFocus) {
        let buffer = getBuffer().slice(), nptValue = input.inputmask._valueGet();
    ...
    function clickEvent(e) {
      function doRadixFocus(clickPos) {
        if (opts.radixFocus && '' !== opts.radixPoint) {
          ...
          if (void 0 === vps[clickPos] || vps[clickPos].input === getPlaceholder(clickPos)) {
            ...
            const radixPos = $.inArray(opts.radixPoint, getBuffer());
            if (-1 !== radixPos) {
              for (const vp in vps) if (vp > radixPos && vps[vp].input !== getPlaceholder(vp)) return !1;
              return !0;
            }
          }
        }
      ...
      if (document.activeElement === input) {
        const selectedCaret = caret(input);
        if (selectedCaret.begin === selectedCaret.end) if (doRadixFocus(selectedCaret.begin)) caret(input, $.inArray(opts.radixPoint, getBuffer())); else {
          let clickPosition = selectedCaret.begin, lvclickPosition = getLastValidPosition(clickPosition), lastPosition = seekNext(lvclickPosition);
    ...
    function cutEvent(e) {
      skipInputEvent = !0;
      let input = this, $input = $(input), pos = caret(input);
      if (isRTL) {
        let clipboardData = window.clipboardData || e.originalEvent.clipboardData, clipData = clipboardData.getData('text').split('').reverse().join('');
    ...
    function blurEvent(e) {
      ...
      if (input.inputmask) {
        let nptValue = input.inputmask._valueGet(), buffer = getBuffer().slice();
        undoValue !== buffer.join('') && setTimeout(() => {
          $input.trigger('change'), undoValue = buffer.join('');
        }, 0), '' !== nptValue && (opts.clearMaskOnLostFocus && (-1 === getLastValidPosition() && nptValue === getBufferTemplate().join('') ? buffer = [] : clearOptionalTail(buffer)),
                isComplete(buffer) === !1 && (setTimeout(() => {
    ...
    function mask(elem) {
      el = elem, $el = $(el), opts.showTooltip && (el.title = getMaskSet().mask), ('rtl' === el.dir || opts.rightAlign) && (el.style.textAlign = 'right'),
            ...
            ('INPUT' === el.tagName && isInputTypeSupported(el.getAttribute('type')) || el.isContentEditable) && ($(el.form).on('submit', () => {
      ...
      const initialValue = $.isFunction(opts.onBeforeMask) ? opts.onBeforeMask.call(el, el.inputmask._valueGet(), opts) || el.inputmask._valueGet() : el.inputmask._valueGet();
      checkVal(el, !0, !1, initialValue.split(''));
      let buffer = getBuffer().slice();
  ...
  Inputmask.prototype = {
    ...
    masksCache: {},
    mask: function(el) {
      const scopedOpts = $.extend(!0, {}, this.opts);
      importAttributeOptions(el, scopedOpts, $.extend(!0, {}, this.userOptions));
      const maskset = generateMaskSet(scopedOpts, this.noMasksCache);
    ...
    'dd/mm/yyyy': {
      ...
      regex: {
        ...
        val2pre: function(separator) {
          const escapedSeparator = Inputmask.escapeRegex.call(this, separator);
      ...
      isInYearRange: function(chrs, minyear, maxyear) {
        ...
        let enteredyear = parseInt(chrs.concat(minyear.toString().slice(chrs.length))), enteredyear2 = parseInt(chrs.concat(maxyear.toString().slice(chrs.length)));
      ...
      determinebaseyear: function(minyear, maxyear, hint) {
        ...
        if (currentyear > maxyear) {
          ...
          const maxxYear = maxYearPrefix + maxYearPostfix;
      ...
      getFrontValue: function(mask, buffer, opts) {
        for (var start = 0, length = 0, i = 0; i < mask.length && '2' !== mask.charAt(i); i++) {
          const definition = opts.definitions[mask.charAt(i)];
      ...
      definitions: {
        '1': {
          validator: function(chrs, maskset, pos, strict, opts) {
            let isValid = opts.regex.val1.test(chrs);
          ...
          prevalidator: [{
            validator: function(chrs, maskset, pos, strict, opts) {
              ...
              let isValid = 1 === pchrs.length ? opts.regex.val1pre.test(pchrs) : opts.regex.val1.test(pchrs);
              if (!strict && !isValid) {
        ...
        '2': {
          validator: function(chrs, maskset, pos, strict, opts) {
            let frontValue = opts.getFrontValue(maskset.mask, maskset.buffer, opts);
            -1 !== frontValue.indexOf(opts.placeholder[0]) && (frontValue = '01' + opts.separator);
            let isValid = opts.regex.val2(opts.separator).test(frontValue + chrs);
            ...
            if (opts.mask.indexOf('2') === opts.mask.length - 1 && isValid) {
              const dayMonthValue = maskset.buffer.join('').substr(4, 4) + chrs;
              if (dayMonthValue !== opts.leapday) return !0;
              const year = parseInt(maskset.buffer.join('').substr(0, 4), 10);
          ...
          prevalidator: [{
            validator: function(chrs, maskset, pos, strict, opts) {
              ...
              let frontValue = opts.getFrontValue(maskset.mask, maskset.buffer, opts);
              -1 !== frontValue.indexOf(opts.placeholder[0]) && (frontValue = '01' + opts.separator);
              let isValid = 1 === chrs.length ? opts.regex.val2pre(opts.separator).test(frontValue + chrs) : opts.regex.val2(opts.separator).test(frontValue + chrs);
        ...
        'y': {
          validator: function(chrs, maskset, pos, strict, opts) {
            if (opts.isInYearRange(chrs, opts.yearrange.minyear, opts.yearrange.maxyear)) {
              const dayMonthValue = maskset.buffer.join('').substr(0, 6);
              if (dayMonthValue !== opts.leapday) return !0;
              const year = parseInt(chrs, 10);
              return year % 4 === 0 ? year % 100 === 0 ? year % 400 === 0 ? !0 : !1 : !0 : !1;
            }
            return !1;
          },
          ...
          prevalidator: [{
            validator: function(chrs, maskset, pos, strict, opts) {
              let isValid = opts.isInYearRange(chrs, opts.yearrange.minyear, opts.yearrange.maxyear);
              if (!strict && !isValid) {
                let yearPrefix = opts.determinebaseyear(opts.yearrange.minyear, opts.yearrange.maxyear, chrs + '0').toString().slice(0, 1);
            ...
            validator: function(chrs, maskset, pos, strict, opts) {
              let isValid = opts.isInYearRange(chrs, opts.yearrange.minyear, opts.yearrange.maxyear);
              if (!strict && !isValid) {
                let yearPrefix = opts.determinebaseyear(opts.yearrange.minyear, opts.yearrange.maxyear, chrs).toString().slice(0, 2);
      ...
      definitions: {
        h: {
          validator: function(chrs, maskset, pos, strict, opts) {
            ...
            let isValid = opts.regex.hrs.test(chrs);
          ...
          prevalidator: [{
            validator: function(chrs, maskset, pos, strict, opts) {
              let isValid = opts.regex.hrspre.test(chrs);
        ...
        s: {
          ...
          prevalidator: [{
            validator: function(chrs, maskset, pos, strict, opts) {
              let isValid = opts.regex.mspre.test(chrs);
              ...
              return {
                validator: function(chrs, maskset, pos, strict, opts) {
                  if (opts.regex['urlpre' + (j + 1)]) {
                      ...
                      let isValid = opts.regex['urlpre' + (j + 1)].test(tmp);
                      if (!strict && !isValid) {
    ...
    numeric: {
      mask: function(opts) {
        ...
        if (0 !== opts.repeat && isNaN(opts.integerDigits) && (opts.integerDigits = opts.repeat),
          ...
          let seps = Math.floor(opts.integerDigits / opts.groupSize), mod = opts.integerDigits % opts.groupSize;
        ...
        let mask = autoEscape(opts.prefix);
      ...
      postFormat: function(buffer, pos, reformatOnly, opts) {
        ...
        let i, l, suffixStripped = !1;
        buffer.length >= opts.suffix.length && buffer.join('').indexOf(opts.suffix) === buffer.length - opts.suffix.length && (buffer.length = buffer.length - opts.suffix.length,
                suffixStripped = !0), pos = pos >= buffer.length ? buffer.length - 1 : pos < opts.prefix.length ? opts.prefix.length : pos;
        let needsRefresh = !1, charAtPos = buffer[pos];
        ...
        const cbuf = buffer.slice();
        charAtPos === opts.groupSeparator && (cbuf.splice(pos--, 1), charAtPos = cbuf[pos]),
                reformatOnly ? charAtPos !== opts.radixPoint && (cbuf[pos] = '?') : cbuf.splice(pos, 0, '?');
        let bufVal = cbuf.join(''), bufValOrigin = bufVal;
        if (bufVal.length > 0 && opts.autoGroup || reformatOnly && -1 !== bufVal.indexOf(opts.groupSeparator)) {
          const escapedGroupSeparator = Inputmask.escapeRegex(opts.groupSeparator);
          needsRefresh = 0 === bufVal.indexOf(opts.groupSeparator), bufVal = bufVal.replace(new RegExp(escapedGroupSeparator, 'g'), '');
          const radixSplit = bufVal.split(opts.radixPoint);
          if (bufVal = '' === opts.radixPoint ? bufVal : radixSplit[0], bufVal !== opts.prefix + '?0' && bufVal.length >= opts.groupSize + opts.prefix.length) for (let reg = new RegExp('([-+]?[\\d?]+)([\\d?]{' + opts.groupSize + '})'); reg.test(bufVal); ) bufVal = bufVal.replace(reg, '$1' + opts.groupSeparator + '$2'),
                    bufVal = bufVal.replace(opts.groupSeparator + opts.groupSeparator, opts.groupSeparator);
          '' !== opts.radixPoint && radixSplit.length > 1 && (bufVal += opts.radixPoint + radixSplit[1]);
        }
        for (needsRefresh = bufValOrigin !== bufVal, buffer.length = bufVal.length, i = 0,
                l = bufVal.length; l > i; i++) buffer[i] = bufVal.charAt(i);
        let newPos = $.inArray('?', buffer);
      ...
      onBeforeWrite: function(e, buffer, caretPos, opts) {
        if (e && ('blur' === e.type || 'checkval' === e.type)) {
          let maskedValue = buffer.join(''), processValue = maskedValue.replace(opts.prefix, '');
          ...
          if (opts.numericInput !== !0) {
            let tmpBufSplit = '' !== opts.radixPoint ? buffer.join('').split(opts.radixPoint) : [buffer.join('')], matchRslt = tmpBufSplit[0].match(opts.regex.integerPart(opts)), matchRsltDigits = 2 === tmpBufSplit.length ? tmpBufSplit[1].match(opts.regex.integerNPart(opts)) : void 0;
            if (matchRslt) {
              ...
              const radixPosition = $.inArray(opts.radixPoint, buffer);
              if (-1 !== radixPosition) {
                if (isFinite(opts.digits) && !opts.digitsOptional) {
                  for (let i = 1; i <= opts.digits; i++) (void 0 === buffer[radixPosition + i] || buffer[radixPosition + i] === opts.placeholder.charAt(0)) && (buffer[radixPosition + i] = '0');
                  return {
        ...
        if (opts.autoGroup) {
          const rslt = opts.postFormat(buffer, opts.numericInput ? caretPos : caretPos - 1, !0, opts);
      ...
      signHandler: function(chrs, maskset, pos, strict, opts) {
        if (!strict && opts.allowMinus && '-' === chrs || opts.allowPlus && '+' === chrs) {
          const matchRslt = maskset.buffer.join('').match(opts.regex.integerPart(opts));
      ...
      radixHandler: function(chrs, maskset, pos, strict, opts) {
        if (!strict && (-1 !== $.inArray(chrs, [',', '.']) && (chrs = opts.radixPoint),
          ...
          let radixPos = $.inArray(opts.radixPoint, maskset.buffer), integerValue = maskset.buffer.join('').match(opts.regex.integerPart(opts));
      ...
      leadingZeroHandler: function(chrs, maskset, pos, strict, opts) {
        if (opts.numericInput === !0) {
          ...
          let matchRslt = maskset.buffer.join('').match(opts.regex.integerNPart(opts)), radixPosition = $.inArray(opts.radixPoint, maskset.buffer);
          if (matchRslt && !strict && (-1 === radixPosition || radixPosition >= pos)) if (0 === matchRslt[0].indexOf('0')) {
            ...
            let _radixPosition = $.inArray(opts.radixPoint, maskset._buffer), digitsMatch = maskset._buffer && maskset.buffer.slice(radixPosition).join('') === maskset._buffer.slice(_radixPosition).join('') || 0 === parseInt(maskset.buffer.slice(radixPosition + 1).join('')), integerMatch = maskset._buffer && maskset.buffer.slice(matchRslt.index, radixPosition).join('') === maskset._buffer.slice(opts.prefix.length, _radixPosition).join('') || '0' === maskset.buffer.slice(matchRslt.index, radixPosition).join('');
      ...
      postValidation: function(buffer, opts) {
        let isValid = !0, maskedValue = buffer.join(''), processValue = maskedValue.replace(opts.prefix, '');
      ...
      definitions: {
        '~': {
          validator: function(chrs, maskset, pos, strict, opts) {
            let isValid = opts.signHandler(chrs, maskset, pos, strict, opts);
            if (!isValid && (isValid = opts.radixHandler(chrs, maskset, pos, strict, opts),
                        !isValid && (isValid = strict ? new RegExp('[0-9' + Inputmask.escapeRegex(opts.groupSeparator) + ']').test(chrs) : new RegExp('[0-9]').test(chrs),
                        isValid === !0 && (isValid = opts.leadingZeroHandler(chrs, maskset, pos, strict, opts),
                        isValid === !0)))) {
              const radixPosition = $.inArray(opts.radixPoint, maskset.buffer);
        ...
        ':': {
          validator: function(chrs, maskset, pos, strict, opts) {
            let isValid = opts.signHandler(chrs, maskset, pos, strict, opts);
            if (!isValid) {
              const radix = '[' + Inputmask.escapeRegex(opts.radixPoint) + ',\\.]';
      ...
      isComplete: function(buffer, opts) {
        let maskedValue = buffer.join(''), bufClone = buffer.slice();
      ...
      onBeforeMask: function(initialValue, opts) {
        if ('' !== opts.radixPoint && isFinite(initialValue)) initialValue = initialValue.toString().replace('.', opts.radixPoint); else {
          let kommaMatches = initialValue.match(/,/g), dotMatches = initialValue.match(/\./g);
        ...
        if (0 === opts.digits && (-1 !== initialValue.indexOf('.') ? initialValue = initialValue.substring(0, initialValue.indexOf('.')) : -1 !== initialValue.indexOf(',') && (initialValue = initialValue.substring(0, initialValue.indexOf(',')))),
          ...
          let valueParts = initialValue.split(opts.radixPoint), decPart = valueParts[1].match(new RegExp('\\d*'))[0];
          if (parseInt(opts.digits) < decPart.toString().length) {
            const digitsFactor = Math.pow(10, parseInt(opts.digits));
      ...
      canClearPosition: function(maskset, position, lvp, strict, opts) {
        ...
        if (canClear && isFinite(positionInput)) {
          let matchRslt, radixPos = $.inArray(opts.radixPoint, maskset.buffer), radixInjection = !1;
          if (void 0 === maskset.validPositions[radixPos] && (maskset.validPositions[radixPos] = {
            ...
            let pos = position + 1, isNull = null == matchRslt || 0 === parseInt(matchRslt[0].replace(new RegExp(Inputmask.escapeRegex(opts.groupSeparator), 'g'), ''));
            if (isNull) for (;maskset.validPositions[pos] && (maskset.validPositions[pos].input === opts.groupSeparator || '0' === maskset.validPositions[pos].input); ) delete maskset.validPositions[pos],
                        pos++;
          }
          const buffer = [];
          for (const vp in maskset.validPositions) void 0 !== maskset.validPositions[vp].input && buffer.push(maskset.validPositions[vp].input);
          if (radixInjection && delete maskset.validPositions[radixPos], radixPos > 0) {
            const bufVal = buffer.join('');
            if (matchRslt = bufVal.match(opts.regex.integerNPart(opts)), matchRslt && radixPos >= position) if (0 === matchRslt[0].indexOf('0')) canClear = matchRslt.index !== position || '0' === opts.placeholder; else {
              let intPart = parseInt(matchRslt[0].replace(new RegExp(Inputmask.escapeRegex(opts.groupSeparator), 'g'), '')), radixPart = parseInt(bufVal.split(opts.radixPoint)[1]);
  ...
  return Inputmask.extendAliases({
    phone: {
      ...
      onBeforeMask: function(value, opts) {
        let processedValue = value.replace(/^0/g, '');
        ...
        r: {
          validator: function(chrs, maskset, pos, strict, opts) {
            ...
            function analyseRegex() {
              let match, m, currentToken = new RegexToken(), opengroups = [];
              ...
              case '*':
                ...
                if (quantifierToken.quantifier = {
                  ...
                  let matches = opengroups[opengroups.length - 1].matches;
            ...
            function validateRegexToken(token, fromGroup) {
              ...
              for (let mndx = 0; mndx < token.matches.length; mndx++) {
                const matchToken = token.matches[mndx];
                if (matchToken.isGroup === !0) isvalid = validateRegexToken(matchToken, !0); else if (matchToken.isQuantifier === !0) {
                  let crrntndx = $.inArray(matchToken, token.matches), matchGroup = token.matches[crrntndx - 1], regexPartBak = regexPart;
                  if (isNaN(matchToken.quantifier.max)) {
                    for (;matchToken.repeaterPart && matchToken.repeaterPart !== regexPart && matchToken.repeaterPart.length > regexPart.length && !(isvalid = validateRegexToken(matchGroup, !0)); ) ;
                    isvalid = isvalid || validateRegexToken(matchGroup, !0), isvalid && (matchToken.repeaterPart = regexPart),
                                        regexPart = regexPartBak + matchToken.quantifier.max;
                  } else {
                    for (let i = 0, qm = matchToken.quantifier.max - 1; qm > i && !(isvalid = validateRegexToken(matchGroup, !0)); i++) ;
                    regexPart = regexPartBak + '{' + matchToken.quantifier.min + ',' + matchToken.quantifier.max + '}';
                  }
                } else if (void 0 !== matchToken.matches) for (let k = 0; k < matchToken.length && !(isvalid = validateRegexToken(matchToken[k], fromGroup)); k++) ; else {
                  var testExp;
                  if ('[' == matchToken.charAt(0)) {
                    ...
                    } else for (let l = 0, tl = matchToken.length; tl > l; l++) if ('\\' !== matchToken.charAt(l)) {
            ...
            for (var bufferStr = cbuffer.join(''), i = 0; i < opts.regexTokens.length; i++) {
              const regexToken = opts.regexTokens[i];
