...
(function($, window, undefined) {
  ...
  $(document).ready(() => {
    ...
    public_vars.$horizontalMenu.find('.horizontal-mobile-menu a').on('click', function(ev)		{
      ...
      let $menu = public_vars.$horizontalMenu.find('.navbar-nav'),
      ...
      $('.slider').each((i, el) => {
        ...
        if(is_range)				{
          $this.slider({
            ...
            step: step,
            slide: function(e, ui)						{
              let min_val = (prefix ? prefix : '') + ui.values[0] + (postfix ? postfix : ''),
  ...
  if($.inArray(params[1], ['min', 'max', 'minlength', 'maxlength', 'equalTo']) != -1)							{
      ...
      $('.form-wizard').each((i, el) => {
        let $this = $(el),
          ...
          _index = $this.find('> ul > li.active').index();
				...
				// Validation
        const checkFormWizardValidaion = function(tab, navigation, index)					{
    ...
    public_vars.$body.on('click', '.modal[data-backdrop="static"]', function(ev)		{
      if( $(ev.target).is('.modal') )			{
        let $modal_dialog = $(this).find('.modal-dialog .modal-content'),
    ...
    if(public_vars.$sidebarUserEnv.length)		{
      let $su_normal = public_vars.$sidebarUserEnv.find('.sui-normal'),
  ...
  $input.val(max);
    ...
    if($search_results_env.length)		{
      let $sr_nav_tabs = $search_results_env.find('.nav-tabs li'),
...
function setup_sidebar_menu() {
  ...
  if(public_vars.$mainMenu.hasClass('auto-inherit-active-class'))	{
	...
	// Search Input
  let $search_input = public_vars.$mainMenu.find('#search input[type="text"]'),
...
function menu_do_expand($submenu, $this, options) {
  ...
  if ( ! public_vars.$mainMenu.hasClass('multiple-expanded') ) {
    $this.siblings( '.has-sub' ).not( $this ).each((i, el) => {
      const $sub = $(el).find( '> ul' );
