 ...
 */
      ...
      XHR.prototype.doWrite = function(data, fn) {
        ...
        const req = this.request({method: 'POST', data: data, isBinary: isBinary});
        const self = this;
        req.on('success', fn);
        req.on('error', (err) => {
 ...
 */
      ...
      Request.prototype.create = function() {
        const opts = {agent: this.agent, xdomain: this.xd, xscheme: this.xs, enablesXDR: this.enablesXDR};
        ...
        for (let i = 0, l = total; i < l; i++) {
          ...
          // always create a new object (GH-437)
                ...
                if (self.perMessageDeflate) {
                  const len = 'string' == typeof data ? global.Buffer.byteLength(data) : data.length;
 ...
 */
      ...
      const blobSupportsArrayBufferView = blobSupported && (function() {
        try {
 ...
 */
    ...
    exports.encodePacket = function(packet, supportsBinary, utf8encode, callback) {
      ...
      const data = (packet.data === undefined)
    ? undefined
 ...
 */
    ...
    exports.encodePayload = function(packets, supportsBinary, callback) {
      ...
      function encodeOne(packet, doneCallback) {
        exports.encodePacket(packet, !isBinary ? false : supportsBinary, true, (message) => {
    ...
    exports.encodePayloadAsBlob = function(packets, callback) {
      function encodeOne(packet, doneCallback) {
        exports.encodePacket(packet, true, true, (encoded) => {
          ...
          const len = (encoded instanceof ArrayBuffer)
        ? encoded.byteLength
        : encoded.size;
          ...
          if (Blob) {
            const blob = new Blob([binaryIdentifier.buffer, lengthAry.buffer, encoded]);
 ...
 */
  ...
  function parse(str) {
    ...
    const match = /^((?:\d+)?\.?\d+) *(milliseconds?|msecs?|ms|seconds?|secs?|s|minutes?|mins?|m|hours?|hrs?|h|days?|d|years?|yrs?|y)?$/i.exec(str);
    if (!match) return;
    ...
    exports.reconstructPacket = function(packet, buffers) {
      ...
      function _reconstructPacket(data) {
        if (data && data._placeholder) {
          const buf = buffers[data.num]; // appropriate buffer (should be natural order anyway)
          return buf;
        } else if (isArray(data)) {
          for (let i = 0; i < data.length; i++) {
    ...
    // implementations are spec-compliant. Based on work by Ken Snyder.
        ...
        // Test `JSON.stringify`.
            if (name == 'json-stringify') {
              let stringify = exports.stringify, stringifySupported = typeof stringify == 'function' && isExtended;
          ...
          if (!(isProperty = objectProto.hasOwnProperty)) {
            ...
            // supports the mutable *proto* property.
              ...
              // unsafe transformation by the Closure Compiler.
                  let original = this.__proto__, result = property in (this.__proto__ = null, this);
      ...
      // level of the output.
        ...
        // `Quote(value)` operation defined in ES 5.1 section 15.12.3.
            ...
            const quote = function(value) {
              let result = '"', index = 0, length = value.length, useCharIndex = !charIndexBuggy || length > 10;
      ...
      // Public: Parses a JSON source string.
        ...
        // literal, or Boolean literal.
            const lex = function() {
              let source = Source, length = source.length, value, begin, position, isSigned, charCode;
              while (Index < length) {
        ...
        // `Walk(holder, name)` operation defined in ES 5.1 section 15.12.2.
            var walk = function(source, property, callback) {
              let value = source[property], length;
              if (typeof value == 'object' && value) {
